	...
	public Set<Book> getBookListByFilter(Map<String, List<String>> filter) {
		...
		if (booksByFilter.contains("publisher")) {
			for (int j = 0; j < filter.get("publisher").size(); j++) {
				...
				String sql = "SELECT b_bookId, b_name, b_unitPrice, b_author, b_description, b_publisher, b_category, b_unitsInStock, b_releaseDate"
						+ ", b_condition FROM book where b_publisher LIKE '%' || ? || '%'";
		...
		if (booksByFilter.contains("category")) {
			for (int i = 0; i < filter.get("category").size(); i++) {
				...
				String sql = "SELECT b_bookId, b_name, b_unitPrice, b_author, b_description, b_publisher, b_category, b_unitsInStock, b_releaseDate"
						+ ", b_condition FROM book where b_category LIKE '%' || ? || '%'";
